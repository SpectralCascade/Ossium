...
void StickFighter::OnInit(Renderer* renderer, int layer)
{
    ...
    stickman->GetTransform()->SetWorldPosition(Point(renderer->GetWindow()->GetWidth() / 3, renderer->GetWindow()->GetHeight() / 2));
...
void StickFighter::Update()
{
    ...
    {
        stickman->transform->worldPosition->y -= 5;
        moving = true;
    }
    ...
    {
        stickman->transform->worldPosition->x -= 5;
        moving = true;
    }
...
void MouseFollower::Update()
{
    ...
    {
        GetTransform()->SetLocalPosition(Point(x, y));
    }
}
